...
def import_clinico():
	df = pd.read_csv('%s/clinical_SMC_cohort.txt' % fi_dir, sep='\t')
...
cp2 = pd.read_csv('%s/SMC_clinical_data_InvasionMetastasisOnly.txt' % fi_dir, sep='\t')
	...
	for ref, df in zip(references, [T_ip, M_ip, TM_ip, TM_NET_ip]):
		...
		for ML in ML_algorithms:
			...
			if ML == 'LogisticRegression':
				...
				param_grid = {'penalty': ['l2'], 'max_iter': [1e10], 'solver': ['lbfgs'], 'C': np.arange(0.1, 1, 0.1),
...
# print output
output.to_csv('%s/FINAL_cross_validation_performance_SMC_cohort_%s.txt'%(fo_dir, test_type), index=False, sep='\t')
