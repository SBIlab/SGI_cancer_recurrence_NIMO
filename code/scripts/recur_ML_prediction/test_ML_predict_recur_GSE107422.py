...
for i in range(number_of_cv_iterations):
    ...
    train_samples = random.sample(t_sample.loc[t_sample["Recur"] == 1, "Sample ID"],
                                  np.int(len(t_sample.loc[t_sample["Recur"] == 1, "Sample ID"]) * training_set))
    test_samples = []
    for sample in t_sample.loc[t_sample["Recur"] == 1, "Sample ID"]:
        if not sample in train_samples:
            test_samples.append(sample)
    train_samples += list(random.sample(t_sample.loc[t_sample["Recur"] == 0, "Sample ID"], len(train_samples)))
    test_samples += list(random.sample(
        t_sample.loc[(t_sample["Recur"] == 0) & (~t_sample["Sample ID"].isin(train_samples)), "Sample ID"],
        len(test_samples)))
...
# print output
output_df.to_csv(
    '%s/FINAL_cross_validation_performance_GSE107422_%s.txt' % (fo_dir,test_type), index=False, sep='\t')
